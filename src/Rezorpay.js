...
function RazorpaySection() {
  ...
  const [amount] = useState(99); // Fixed amount 
    ...
    try {
      ...
      try {
        const verifyResponse = await paymentAPI.verifyPayment({
          paymentId: `pay_${Date.now()}`, // Simulated payment ID
          orderId: orderResponse.payment.orderId,
          signature: "simulated_signature", // In production, this would come from Razorpay
        });

        alert(
          `✅ Payment Successful!\nPayment ID: ${verifyResponse.payment.paymentId}\nOrder ID: ${verifyResponse.payment.orderId}\nAmount: ₹${amount}`
        );
        setLoading(false);
